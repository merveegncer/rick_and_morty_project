...
class _GetAndShowEpisodesState extends State<GetAndShowEpisodes> {
  @override
  void initState() {
    getEpisodes(EndPoint.episode.ep);
    super.initState();
    pageNum = 1;
    seasonList = s1;
  }

  String _nextPageUrl = EndPoint.episode.ep;
  String _previousPageUrl = EndPoint.episode.ep;
  var pageNum = 0;
    ...
    try {
      for (var i = 1; i < 4; i++) {
        ...
        if (response.statusCode == 200) {
          ...
          _nextPageUrl =
              data['info']['next'] ?? "${EndPoint.episode.ep}?page=3";
          _previousPageUrl =
              data['info']['prev'] ?? "${EndPoint.episode.ep}?page=1";
